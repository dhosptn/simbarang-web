...
<body class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center space-x-6">
          <a href="<?= base_url('/') ?>" class="text-white/80 hover:text-white transition-colors duration-200">
            <i class="fas fa-chart-bar mr-2"></i>Home
          </a>
  ...
  <!-- Tambahkan ini setelah tombol hamburger -->
  <div id="mobileMenu" class="md:hidden hidden flex-col bg-white/10 backdrop-blur-md border-t border-white/20 px-4 py-3 space-y-2">
    <a href="<?= base_url('/') ?>" class="text-white/80 hover:text-white transition-colors duration-200 block">
      <i class="fas fa-chart-bar mr-2"></i>Home
    </a>
    <form action="<?= base_url('logout') ?>" method="POST" class="inline">
      <?= csrf_field() ?>
      <button type="submit" class="text-white/80 hover:text-white transition-colors duration-200 block w-full text-left">
        <i class="fas fa-sign-out-alt mr-2"></i>LogOut
      </button>
    </form>
  </div>
  ...
  <div class="container mx-auto px-4 py-8 max-w-7xl relative">
    ...
    $low_stock = array_filter($barang, function ($item) {
      return $item['stok'] < 5;
    });
    $categories = array_unique(array_column($barang, 'nama_kategori'));
    $locations = array_unique(array_column($barang, 'nama_lokasi'));
    ?>
      ...
      <div class="flex flex-col md:flex-row md:items-end gap-4">
        ...
        <div class="flex-1">
          ...
          <select id="categoryFilter"
            class="block w-full px-3 py-2 bg-white/5 border border-white/20 rounded-lg text-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-200"
              ...
              <?php foreach ($barang as $b): ?>
                <tr class="hover:bg-white/5 transition-all duration-200 group">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div class="w-10 h-10 bg-gradient-to-r <?= getCategoryColor($b['nama_kategori']) ?> rounded-lg flex items-center justify-center mr-3">
                        <i class="<?= getCategoryIcon($b['nama_kategori']) ?> text-white text-sm"></i>
                      </div>
                      <div>
                        <div class="text-sm font-medium text-white"><?= esc($b['nama_barang']) ?></div>
                        <div class="text-sm text-white/60"><?= ucfirst(esc($b['nama_kategori'])) ?> item</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gradient-to-r <?= getCategoryColor($b['nama_kategori']) ?> text-white">
                      <i class="<?= getCategoryIcon($b['nama_kategori']) ?> mr-1"></i>
                      <?= ucfirst(esc($b['nama_kategori'])) ?>
                    </span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <span class="<?= $b['stok'] < 5 ? 'text-red-400 animate-pulse' : 'text-green-400' ?> font-bold text-lg">
                        <?= $b['stok'] ?>
                      </span>
                      <div class="ml-2 w-16 bg-white/20 rounded-full h-2">
                        <?php
                        $percentage = min(100, ($b['stok'] / 50) * 100);
                        $color = $b['stok'] < 5 ? 'from-red-400 to-red-500' : 'from-green-400 to-green-500';
                        ?>
                        <div class="bg-gradient-to-r <?= $color ?> h-2 rounded-full" style="width: <?= $percentage ?>%"></div>
                      </div>
                      <?php if ($b['stok'] < 5): ?>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div class="flex space-x-3">
                      <a href="/barang/edit/<?= $b['id'] ?>" class="text-blue-400 hover:text-blue-300 transform hover:scale-110 transition-all duration-200" title="Edit">
                        <i class="fas fa-edit text-lg"></i>
                      </a>
                      <button class="text-green-400 hover:text-green-300 transform hover:scale-110 transition-all duration-200" title="View">
                        <i class="fas fa-eye text-lg"></i>
                      </button>
                      <a href="/barang/delete/<?= $b['id'] ?>" onclick="return confirm('Yakin ingin menghapus barang ini?')" class="text-red-400 hover:text-red-300 transform hover:scale-110 transition-all duration-200" title="Delete">
    ...
    <div class="container mx-auto px-4 py-8">
      <div class="flex flex-col md:flex-row justify-between items-center">
        <div class="flex items-center space-x-3 mb-4 md:mb-0">
          ...
          <span class="text-white font-semibold">SimBarang</span>
        </div>
        <div class="text-white/60 text-sm">
          © 2024 SimBarang. All rights reserved.
        </div>
      </div>
    </div>
  </footer>
